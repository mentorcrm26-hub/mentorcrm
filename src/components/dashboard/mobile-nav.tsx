...
export function MobileNav({ role, tenantName, tenantId, isSuperAdmin }: { role: string | null, tenantName: string | null, tenantId: string | null, isSuperAdmin?: boolean }) {
    ...
    if (!mounted) {
        return (
            <button className="p-2 -mr-2 text-zinc-600 dark:text-zinc-300 rounded-md">
    ...
    return (
        <>
            ...
            {createPortal(
                <div className={`fixed inset-0 z-[9999] lg:hidden ${isOpen ? 'visible' : 'invisible'}`}>
                    {/* Backdrop */}
                    <div
                    ...
                    {/* Drawer */}
                    <div className={`absolute inset-y-0 right-0 w-[280px] sm:w-[320px] bg-white dark:bg-zinc-950 border-l border-zinc-200 dark:border-white/10 shadow-2xl transition-transform duration-300 ease-in-out flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
