...
import { X, Send, MessageSquareText, Mail, Loader2, Megaphone, AlertCircle, ShieldAlert, Clock } from 'lucide-react'
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className={`bg-white dark:bg-zinc-950 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-zinc-200 dark:border-zinc-800`}>
                ...
                <div className="p-6 overflow-y-auto max-h-[70vh]">
                    {isSuccess ? (
                        ...
                        <div className="space-y-5">
                                    ...
                                    <div className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-900/50 rounded-lg border border-zinc-200 dark:border-zinc-800">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <input
                                                ...
                                                className="w-16 p-1 text-center bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-700 rounded-md text-sm font-semibold shadow-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                                            />
                            ...
                            {/* Message Area */}
                            <div>
                                <label className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 mb-2 flex justify-between">
                                    Message Composition
                                    <span className="text-xs font-normal text-zinc-500">Variables available: {'{name}'}</span>
                                </label>
                                <textarea
                                    value={message}
                                    ...
                                    rows={5}
                                    className="w-full p-4 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 text-zinc-900 dark:text-white resize-none"
                                />

                                <div className="flex gap-2 mt-2">
                                    <button
                                        onClick={() => setMessage(`Hello {name},\n\nJust a quick broadcast message to say hi!`)}
                                        className="px-3 py-1.5 text-xs font-medium bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 rounded-full hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors"
                                    >
                ...
                {!isSuccess && (
                    <div className="px-6 py-4 bg-zinc-50 dark:bg-zinc-900/50 border-t border-zinc-100 dark:border-zinc-800 flex justify-between items-center gap-3">
                        <button
                            onClick={onClose}
