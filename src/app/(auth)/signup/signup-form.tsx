'use client';

/**
 * ************ By Inova Digital Marketing ***************
 * ******************* inovamkt.io ************************
 * ******************* Paulo Daian ************************
 * *************** contact@inovamkt.io ******************
 */

import React, { useState } from 'react';
import { signup } from '../actions';

import { PasswordInput } from '@/components/ui/password-input';

interface SignupFormProps {
    fullName: string;
    namePlaceholder: string;
    emailLabel: string;
    emailPlaceholder: string;
    phoneLabel: string;
    passwordLabel: string;
    passwordPlaceholder: string;
    terms: string;
    submit: string;
    plan: string;
}

export default function SignupForm({ fullName, namePlaceholder, emailLabel, emailPlaceholder, phoneLabel, passwordLabel, passwordPlaceholder, terms, submit, plan }: SignupFormProps) {
    const [phone, setPhone] = useState('');

    const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        let value = e.target.value.replace(/\D/g, '');
        if (value.length > 10) value = value.slice(0, 10);
        let maskedValue = '';
        if (value.length > 0) {
            maskedValue = '(' + value.slice(0, 3);
            if (value.length > 3) maskedValue += ') ' + value.slice(3, 6);
            if (value.length > 6) maskedValue += '-' + value.slice(6, 10);
        }
        setPhone(maskedValue);
    };

    return (
        <form className="flex flex-col gap-6" action={signup}>
            <input type="hidden" name="plan" value={plan} />

            {/* Name */}
            <div className="flex flex-col gap-3">
                <label className="text-[10px] font-display font-black tracking-[0.2em] text-white/30 uppercase" htmlFor="full_name">{fullName}</label>
                <input
                    className="bg-white/5 border border-white/10 px-6 py-4 text-sm text-white placeholder-white/20 rounded-2xl focus:outline-none focus:border-brand-500/50 focus:bg-white/10 transition-all shadow-inner"
                    id="full_name"
                    name="full_name"
                    type="text"
                    placeholder={namePlaceholder}
                    required
                />
            </div>

            {/* Email */}
            <div className="flex flex-col gap-3">
                <label className="text-[10px] font-display font-black tracking-[0.2em] text-white/30 uppercase" htmlFor="email">{emailLabel}</label>
                <input
                    className="bg-white/5 border border-white/10 px-6 py-4 text-sm text-white placeholder-white/20 rounded-2xl focus:outline-none focus:border-brand-500/50 focus:bg-white/10 transition-all shadow-inner"
                    id="email"
                    name="email"
                    type="email"
                    placeholder={emailPlaceholder}
                    required
                />
            </div>

            {/* Phone (Masked) */}
            <div className="flex flex-col gap-3">
                <label className="text-[10px] font-display font-black tracking-[0.2em] text-white/30 uppercase" htmlFor="phone">{phoneLabel}</label>
                <input
                    className="bg-white/5 border border-white/10 px-6 py-4 text-sm text-white placeholder-white/20 rounded-2xl focus:outline-none focus:border-brand-500/50 focus:bg-white/10 transition-all shadow-inner"
                    id="phone"
                    name="phone"
                    type="text"
                    value={phone}
                    onChange={handlePhoneChange}
                    placeholder="(XXX) XXX-XXXX"
                    required
                />
            </div>

            {/* Password */}
            <div className="flex flex-col gap-3">
                <label className="text-[10px] font-display font-black tracking-[0.2em] text-white/30 uppercase" htmlFor="password">{passwordLabel}</label>
                <PasswordInput
                    id="password"
                    name="password"
                    placeholder={passwordPlaceholder}
                    minLength={6}
                    required
                />
            </div>


            {/* Terms */}
            <div className="flex items-start gap-4 p-5 bg-white/5 border border-white/10 rounded-2xl mt-2 shadow-inner">
                <input type="checkbox" id="terms" name="terms" className="mt-1 h-4 w-4 accent-brand-500 rounded border-white/10 bg-transparent" required />
                <label htmlFor="terms" className="text-[10px] text-white/30 font-display font-black uppercase tracking-[0.05em] leading-relaxed">
                    {terms}
                </label>
            </div>

            {/* Submit */}
            <div className="pt-4">
                <button
                    type="submit"
                    className="group relative flex w-full h-16 items-center justify-center bg-brand-500 hover:bg-brand-600 px-8 text-xs font-display font-black uppercase tracking-[0.4em] text-white transition-all rounded-2xl shadow-[0_10px_30px_rgba(0,112,204,0.4)] active:scale-95 cursor-pointer"
                >
                    {submit}
                </button>
            </div>
        </form>
    );
}
